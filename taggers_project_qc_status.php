...
<?php 
      ...
      if(isset($_POST['submission'])){
		    ...
		    {
		    $sql_query="INSERT INTO `taggers_qc_status`(`project_id`, `project_name`, `cluster_id`, `qc_id`, `total_annotations`, `missed_annotations`, `incorrect_self`, `incorrect_comp`, `incorrect_others`, `precision`, `recall`, `url`, `time`, `date`, `status`,`email`,`T_self_pres`, `T_others_pres`, `T_comp_pres`, `Grade_A_Category`, `Grade_B_Category`, `Grade_C_Category`, `Grade_D_Category`, `Proj_Grade`)
		         VALUES ('$project_id','$project_name','$cluster_id','$qc_id','$total_annotations','$missed_annotations','$incorrect_self','$incorrect_comp','$incorrect_others','$precision','$recall','$url','$time','$date','PENDING','$email','$T_S_P', '$T_O_P', '$T_C_P', '$G_A_C', '$G_B_C', '$G_C_C', '$G_D_C', '$project_grade')";
            ...
            if($runn == true){
				?>
                  <script>
					...
					toastMixin.fire({
					 ...
					 setTimeout("window.open('taggers_project_qc_status.php', '_self');", 2000);
				  </script>
				<?php
			}
			    ...
			    <script>
			    alert("failed");
			    setTimeout("window.open('taggers_project_qc_status.php', '_self');", 1000);
...
<body>
	...
	<div class="page-content">
		...
		<div class="content-wrapper">
			...
			<div class="content-inner">
				...
				<form method="POST" action="taggers_project_qc_status.php">
				    ...
				    <div class="row">
						<div class="col-xl-12">
							...
							<?php
							 ...
							 $sql_location="SELECT * FROM `taggers_qc_status` ORDER BY `date` DESC";
                 $runn=mysqli_query($conn,$sql_location);
                 $count=mysqli_num_rows($runn);
                 if($count != '0')
                 {
                      ...
                      while($data=mysqli_fetch_assoc($runn)){
                          ...
                          ?>
                                <tr id="del_<?php echo $data['id']; ?>">
                                    ...
                                    ?>
                                    <td>ID : <?php echo ucfirst($dpidn['owner_name']); ?><br> Name : <?php echo ucfirst($dpidn['project_name']); ?><br>BY : <?php echo $data['email']; ?></td>
                                    ...
                                    <td><a href="<?php echo $data['url']; ?>" target="_blank">URL</a></td>
                                     ...
                                     <!--status-->
                                    <th>
                                        <span id="qc-status-text-<?php echo $data['id']; ?>">
                                            <?php echo $data['status']; ?>
                                        </span>
                                    
                                        <label class="switch">
                                            <input type="checkbox"
                                                   <?php echo ($data['status'] == 'DONE') ? 'checked' : ''; ?>
                                                   onchange="updateQCStatus('<?php echo $data['id']; ?>', this)">
    ...
    ?>
					...
					<script>
				     ...
				     function updateQCStatus(id, checkbox) {

                                    let newStatus = checkbox.checked ? 'DONE' : 'PENDING';
                                
                                    $.ajax({
                                        type: "POST",
                                        url: "Status/taggers_qc_status.php",
                                        data: {
                                            id: id,
                                            status: newStatus
                                        },
                                        success: function(response) {
                                            ...
                                            document.getElementById("qc-status-text-" + id).innerText = newStatus;
                                
                                            // Snackbar feedback
                                            var x = document.getElementById("snackbar");
                                            x.innerText = "QC Status updated to " + newStatus;
                                            x.className = "show";
				     ...
				     function deleteData(id) {
                                ...
                                $.post("Delete/taggers_qc_status.php", { id: id }, function(response) {
