				...
				?>
                  <script>
					...
					toastMixin.fire({
					 ...
					 setTimeout("window.open('test_t_Q.php', '_self');", 2000);
				  </script>
				<?php
			}
	  }
	...
	<div class="page-content">
		...
		<div class="content-wrapper">
			...
			<div class="content-inner">
				...
				<form method="POST" action="test_t_Q.php">
                              ...
                              while($data=mysqli_fetch_assoc($runn)){
                                  ?>
                                        <tr id="del_<?php echo $data['id']; ?>"> 
                                            ...
                                            <td contenteditable="true" data-id="<?php echo $data['id']; ?>" data-column="concern" onblur="updateCell(this)"><?php echo ucfirst($data['concern']); ?></td>
                                            <td><?php echo $data['date']; ?></td>
                                            
                                            <!--status update-->
                                             <th>
                                                <span id="info-status-text-<?php echo $data['id']; ?>">
                                                    <?php echo ucfirst($data['info_status']); ?>
                                                </span><br>
                                            
                                                <label class="switch">
                                                    <input type="checkbox"
                                                           <?php echo ($data['info_status'] == 'recieved') ? 'checked' : ''; ?>
                                                           onchange="updateInfoStatus('<?php echo $data['id']; ?>', this)">
            ...
            ?>
					...
					<script>
				     ...
				     function updateInfoStatus(id, checkbox) {

                        let newStatus = checkbox.checked ? 'recieved' : 'pending';
                        ...
                        $.ajax({
                            ...
                            success: function (response) {
                                ...
                                // Update text live
                                document.getElementById("info-status-text-" + id).innerText =
                                    newStatus.charAt(0).toUpperCase() + newStatus.slice(1);
                    
                                // Snackbar
                                var x = document.getElementById("snackbar");
                                x.innerText = "Info status updated to " + newStatus;
                                x.className = "show";
				     ...
				     function delete_date(id){
        				        ...
        				        $.ajax({
                                        ...
                                        success: function(data) {
                                          ...
                                          if(data == '0'){
                                              ...
                                              setTimeout(function(){ window.open('test_t_Q.php','_self'); }, 2000);
                                          }else{
                                              var x = document.getElementById("danger");
                                              x.className = "show";
                                              setTimeout(function(){ x.className = x.className.replace("show", ""); }, 2000);
				        ...
				        function updateCell(cell) {
                                        ...
                                        $.ajax({
                                            url: "inline_edit/update_cell.php", // Backend URL for processing
                                            type: "POST",
                                            data: {
                                                id: id,
                                                column: column,
